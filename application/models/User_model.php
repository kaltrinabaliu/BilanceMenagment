...
class User_model extends CI_Model
{
    ...
    {
        $this->db->select('bilance');
        $this->db->where('userId', $userId);
        $query = $this->db->get('tbl_users');

        if ($query->num_rows() > 0) {
            $result = $query->row();
            return $result->bilance;
        } else {
            return 0.0; 
        }
    }
...
public function sendBalance($recipientEmail, $balanceAmount)
{
    $this->db->select('userId, bilance');
    $this->db->where('email', $recipientEmail);
    $query = $this->db->get('tbl_users');

    if ($query->num_rows() > 0) {
        $recipient = $query->row();
        $recipientUserId = $recipient->userId;
        $recipientBalance = $recipient->bilance;

        // Increase the balance of the recipient
        $this->updateUserBalance($recipientUserId, $recipientBalance + $balanceAmount);
